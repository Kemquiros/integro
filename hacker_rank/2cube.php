...
$actual=0;
//-----------
//  Recibe el texto como parametro
//-----------
$_fp = $_POST['text'];
//Divide en lineas el texto
$texto= explode("\n", $_fp);
//-----------
//  Lee el numero de casos de uso
//-----------
$T = intval($texto[$actual]);
$actual++;
$respuesta="";
...
for($t = 0; $t < $T; $t++){
  ...
  //-----------
  list($N, $M) = explode(" ", $texto[$actual]);//La funcion explode es mas eficiente que split
  $actual++;
  //-----------
  //  Por cada operacion
  //-----------
  for($m = 0; $m < $M; $m++){
    $linea = explode(" ", $texto[$actual]);
    $actual++;
    if($linea[0] == "UPDATE"){
      //-----------
      ...
      //array_push($respuesta,$cubo -> getSum()."\n");
      $respuesta.=$cubo -> getSum()."\n";
